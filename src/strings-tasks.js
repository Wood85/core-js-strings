...
function removeFirstOccurrences(str, value) {
  const string = str;
  const substr = value;
  const firstInd = string.indexOf(substr);
  if (firstInd >= 0) {
    return string
      .slice(0, firstInd)
      .concat(string.slice(firstInd + substr.length));
  }
  return string;
}
...
function removeLastOccurrences(str, value) {
  const string = str;
  const substr = value;
  const firstInd = string.lastIndexOf(substr);
  if (firstInd >= 0) {
    return string
      .slice(0, firstInd)
      .concat(string.slice(firstInd + substr.length));
...
function findLongestWord(sentence) {
  ...
  const arrLength = arr.map((w) => w.length);
  const sortedArrLength = arrLength.sort((a, b) => a - b);
  const longestWordLength = sortedArrLength[sortedArrLength.length - 1];
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverseArr = arr.map((i) => i.split('').reverse().join(''));
